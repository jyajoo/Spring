...
@Profile("dev")// dev Profile이 active될 때만 스프링 컨테이너에 포함된다.
public class DevInitData {
  ...
  CommandLineRunner initTestData(MemberService memberService, PasswordEncoder passwordEncoder, BoardService boardService) {
    ...
    return args -> {
      Member user1 = memberService.join("user1", password, "user1@dev.com");
      Member user2 = memberService.join("user2", password, "user2@dev.com");
